...
void hdparse(t_pipex *p) {
    ...
    while (1) {
        ...
        if (buff == NULL) // Gestion de Ctrl+D ou erreur de lecture
            break;
        // Correction : parenthèse fixée et comparaison exacte
        if (ft_strncmp(buff, p->av[2], ft_strlen(p->av[2])) == 0
            && ft_strlen(buff) == ft_strlen(p->av[2]) + 1) { // +1 pour le '\n'
            free(buff);
            break; // Sortie immédiate de la boucle
        }
        ft_putstr_fd(buff, tmp); // Écrit dans le fichier temporaire
        free(buff);
    }
    close(tmp);
    close(fd_std); // Nettoyage
}
//
//void hdparse(t_pipex *p)
//{

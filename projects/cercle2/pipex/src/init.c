...
/*   Updated: 2025/03/18 12:29:54 by akaiissa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pipex pipex_i(int ac, char **av, char **envp)
{
    ...
    if (!pi.pids)
        clean_exit(fail_msg("No more space in your memory, can't stock PIDs.", strerror(errno), "", 1), &pi);
    pi.pipe = malloc(sizeof * pi.pipe * 2 * (pi.cmds_nu - 1));
    if (!pi.pipe)
        clean_exit(fail_msg("No more space in your memory, can't stock pipes.", "", "", 1), &pi);
    pipe_pop(&pi);
    return (pi);
}
